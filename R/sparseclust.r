  ...
  methods = list(
    ...
    calculatePcaReduction=function(nPcs=20, type='counts', name='PCA', use.odgenes=FALSE, n.odgenes=2e3, odgenes=NULL, scale=F,center=T, cells=NULL,fastpath=TRUE,maxit=100) {
      ...
      if(!is.null(cells)) {
        ...
        pcs <- irlba(x[cells,], nv=nPcs, nu=0, center=cm, right_only=FALSE,fastpath=T,fastpath=fastpath,maxit=maxit,reorth=T)
      } else {
        if(center) {
          cm <- Matrix::colMeans(x)
          pcs <- irlba(x, nv=nPcs, nu=0, center=cm, right_only=FALSE,fastpath=T,fastpath=fastpath,maxit=maxit,reorth=T)
        } else {
          pcs <- irlba(x, nv=nPcs, nu=0, right_only=FALSE,fastpath=T,fastpath=fastpath,maxit=maxit,reorth=T)
        }
      }
