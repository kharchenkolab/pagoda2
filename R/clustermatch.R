...
plotJointClusters <- function(r.n, cl, alpha =0.3, main=NULL) {
...
getMNNforP2pair <- function(r1, r2, var.scale =T , k = 30, log.scale=T, center=T, verbose =T, ncomps = 100, plot.projection = F) {
    ## r1 <- p2objs[[1]]
    ## r2 <- p2objs[[2]]
    ## var.scale <- T
    ## k <- 30
