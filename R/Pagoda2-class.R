...
Pagoda2 <- R6::R6Class("Pagoda2",
  ...
  public = list(
    ...
    #' @param backend Storage backend: "memory" (default) or "lstar" (on-disk zarr, out-of-core).
    #' @param backend.dir Optional store path for a disk-backed backend.
    #' @return Invisibly returns self.
    addFacet = function(name, countMatrix, modelType = "plain", featureType = "gene", defaultReduction = "PCA", depth = NULL, backend = c("memory", "lstar"), backend.dir = NULL) .pagoda2_r6_add_facet(self, name = name, countMatrix = countMatrix, modelType = modelType, featureType = featureType, defaultReduction = defaultReduction, depth = depth, backend = backend, backend.dir = backend.dir),
    ...
    plotEmbedding = function(type = NULL, embeddingType = NULL, reduction = NULL, embedding = NULL,
                             ...
                             gene = NULL, facet = NULL, plot.theme = NULL, .legacy.warn = TRUE, ...) {
      args <- list(
        reduction = reduction,
        embedding = embedding,
