...
void LargeVis::load_from_graph_data(std::vector<int>& ef, std::vector<int>& et, std::vector<real>& ew,int nv) 
{
	clean_data();
	n_vertices = nv;
	n_edge=ef.size();
	edge_from=ef; edge_to=et; edge_weight=ew;
	next=std::vector<long long>(ef.size(),-1);
	long long x, y, i, p;
	head = new long long[n_vertices];
	for (i = 0; i < n_vertices; ++i) head[i] = -1;
	for (p = 0; p < n_edge; ++p)
